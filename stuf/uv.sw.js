            ...
            async fetch({
                ...
                try {
                    ...
                    if (r.headers.location) {
                        let rewrittenLocation = t.rewriteUrl(r.headers.location);
                        let locationUrl = new URL(rewrittenLocation, location.origin);
                        if (locationUrl.origin === location.origin && locationUrl.pathname.startsWith(this.config.prefix)) {
                            let redirectSourceUrl = t.sourceUrl(locationUrl.href);
                            let newProxiedPath = location.origin + this.config.prefix + t.encodeUrl(redirectSourceUrl);
                            r.headers.location = newProxiedPath;
                        } else {
                            r.headers.location = rewrittenLocation;
                        }
                    }
                    if (["document", "iframe"].includes(e.destination)) {
...
`;
                    ...
                    if (o.headers.accept === "text/event-stream") r.headers["content-type"] = "text/event-stream";
                    if (crossOriginIsolated) r.headers["Cross-Origin-Embedder-Policy"] = "require-corp";
                    this.emit("response", l);
                    if (l.intercepted) return l.returnValue;

                    let responseBody = r.body;
                    let responseHeaders = new Headers(r.headers);
                    if (!responseHeaders.has("content-type") && r.getHeader("content-type")) {
                        responseHeaders.set("content-type", r.getHeader("content-type"));
                    }
                    ...
                    return ["document", "iframe"].includes(e.destination) ? (console.error(t), T(t, s)) : new Response(void 0, {
                        status: 500
                    })
                }
            }
    ...
    self.addEventListener("fetch", e => {
        let sw;
        try {
            sw = new g(__uv$config);
        } catch {
            return;
        }
        if (!sw.route(e)) return;
        e.respondWith(
            sw.fetch(e).catch(err => {
                if (["document", "iframe"].includes(e.request.destination)) {
                    return T(err, e.request.url);
                }
    ...
    function T(a, e) {
        ...
        if (crossOriginIsolated) s["Cross-Origin-Embedder-Policy"] = "require-corp";
        return new Response(E(String(a), e), {
            status: 500,
            headers: s
        })
    }
